...
### 1.4 上一段对取窗的归因又错了，而且错在同一个地方（2026-08-13 补记）

「结构与因子层同构、需要给它做缓存」**不成立**。真实机制是取窗**顺序**：
`build_factor_snapshot` 开头那个覆盖全 universe 的 dict comprehension 位于
`base_path.exists()` 与三层查表**之前**，于是三层全命中的热运行也要把每一只
取窗并复权一遍。要害在于**这些窗口一个都没被消费**——`windows[code]` 在热
路径上只被读了 `len(group)`（用于 `< 60` 过滤与 `as_of_index`），而
`apply_read_adjustment` 的四条出口都不增删行。所以那 8.77s 是 100% 白做的。

因此修法是把物化推迟到真有消费方时，**不引入任何缓存**。「给窗口做缓存」
不是权衡后被放弃，是机制查清后它自动失效了：缓存一个没人读的东西没有意义。
同会话受控 A/B（15 只 × 32 天 × `--workers 4`，OS 页缓存两侧同样暖）：
...
`apply_read_adjustment` 调用 7230 → 0，父进程 peak working set 155.6 → 153.3 MB，
四趟报告 sha256 逐字节相同。

**冷路径慢了 3.8%，这笔要认。** 冷 code 的取窗谓词现在算两遍（`window_for` 一次、
`_window_row_count` 一次）。审查预判它"可忽略"，实测是可测量的 5.4s。它按每个缓存
目录付一次，换来的是此后每次运行都省，所以这个方向仍然划算——但不要把它写成
"冷路径不变"。

**测量口径本身也有一课**：第一次跑出的冷运行是 153.2s，比 HEAD 的 140.8s 慢 8.8%，
差点被当成 9% 的冷回归。真因是那一趟是本会话里第一个读 9.6M 行 `stock_daily` 的
进程，OS 页缓存全冷，而 HEAD 那趟跑在它后面吃到了暖缓存。**跨会话的绝对耗时不可比，
连同一会话里的先后顺序都会造出一个 9% 的假信号**；本节所有数都是同会话、同缓存
状态下重测的。

配套抽出了 `_window_mask` 作为取窗谓词的**唯一**定义。这是正确性而非整洁：
`as_of_index` 取自行数、却要与随后被评估的那个窗口对齐，两处各写一遍谓词
就是一个「键对不上内容」的静默错误。

**同时补回一道被拆掉的结构性断言**：改动前 v2 循环里的 `windows[code]` 会
对 universe 之外的 code 抛 `KeyError`；换成 `_window_row_count` 之后，这类
code（必然在 `_bar_groups` 里，它们的 base 行就是从那儿算出来的）会被正常
评估并写进返回值，**调用方拿到一份超出自己 universe 的快照且不报错**。
今天不可达——base 快照要么按含 `_universe_fingerprint` 的文件名读回、要么由
`sorted(windows)` / `sorted(bar_groups)` 现算，两条都以 `codes` 为上界。
但打破它的正是本文件 §1 那条前提从「同策略多网格」推广到「多策略」时最自然
的实现：对 union universe 建一次 base 快照、各策略读自己的子集。
`test_a_base_snapshot_wider_than_the_universe_fails_loudly` 把那一刻写死。

**方法论上这是本轮第三次「先测量」推翻「先实施」**：§1.2 假定瓶颈在因子计算
（错，实为分区抖动），§1.3 假定内存 LRU 可行（错 27 倍）并假定取窗需要缓存
（错，实为顺序问题）。三次都是先量了才知道方案本身不该做。
